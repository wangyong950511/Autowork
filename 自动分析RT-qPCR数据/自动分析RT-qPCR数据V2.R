...
####需要手动修改的数据####
# 定义文件路径
filepath_input <- "/Users/wangyong/Desktop/2025-04-24_094618.xls"
# 指定顺序 
sample_order <- c("P4","P7","P9","P11","P32","T4","T7","T9","T11","T32")
...
## 集合目的参数
# 定义结构文件路径
Sample_Gene_input <- '/Users/wangyong/Desktop/通用加样表.xlsx'
# 读取数据
Sample_data <-
...
Gene_long <- Gene_data %>%
  ...
  mutate(Variable = as.character(row_number()))  # 将 Variable 设置为字符型
Sample_long <- Sample_data %>%
  pivot_longer(cols = everything(), names_to = "Variable", values_to = "Sample") %>%
  mutate(Variable = as.character(row_number()))
# 替换到目标表格数据
data <- data %>%
  left_join(Gene_long %>% rename(Well = Variable), by = "Well") %>% # 按照 Well 匹配 Gene_long
  left_join(Sample_long %>% rename(Well = Variable), by = "Well") %>% # 按照 Well 匹配 Sample_long
  mutate(`Target Name` = Gene, `Sample Name` = Sample) %>% # 添加新列
  select(-Gene, -Sample) # 删除多余的中间列
#参考样本设定
